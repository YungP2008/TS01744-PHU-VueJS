...
const handleAddPost = (newPostData) => {
  const postWithAuthor = {
    ...newPostData,
    author: currentUser.value ? currentUser.value.fullName : 'Khách',
  };
  posts.value.unshift(postWithAuthor);
};
...
const handleAddComment = ({ postId, content }) => {
  ...
  if (post) {
    const userName = currentUser.value ? currentUser.value.fullName : 'Khách';
